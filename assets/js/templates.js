const FCXD = {
  nav: { 
    zh: ['首页', '关于我们', '理事会', '章程', '活动与项目', '加入我们', '联系'], 
    en: ['Home', 'About', 'Board', 'Statutes', 'Activities', 'Join us', 'Contact'], 
    de: ['Start', 'Über uns', 'Vorstand', 'Satzung', 'Aktivitäten', 'Mitmachen', 'Kontakt'] 
  },
  paths: ['index.html', 'about.html', 'vorstand.html', 'statutes.html', 'activities.html', 'join.html', 'contact.html'],
  
  header(current) {
    const links = this.paths.map((p, i) => 
      ['zh', 'en', 'de'].map(l => 
        `<a class="lang" data-lang="${l}" ${p === current ? 'aria-current="page"' : ''} href="${p}">${this.nav[l][i]}</a>`
      ).join('')
    ).join('');
  ...
  footer() { 
    return `<footer><div class="wrap"><div class="footer-links"><a href="impressum.html">Impressum</a><a href="privacy.html">Datenschutz / Privacy / 隐私</a></div><p class="lang" data-lang="zh">德国中国新疆友协 · 非注册协会 · 文化交流与友谊</p><p class="lang" data-lang="en">Freundeskreis China-Xinjiang in Deutschland · Unregistered association · Cultural exchange and friendship</p><p class="lang" data-lang="de">Freundeskreis China-Xinjiang in Deutschland · Nichteingetragener Verein · Kultureller Austausch und Freundschaft</p><p>© <span id="year"></span> Freundeskreis China-Xinjiang in Deutschland</p></div></footer>`; 
  },
  
  render(current) { 
    document.querySelector('[data-header]').innerHTML = this.header(current); 
    document.querySelector('[data-footer]').innerHTML = this.footer(); 
